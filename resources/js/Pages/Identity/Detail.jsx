    ...
    <AppLayout>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => router.get(route('identity.index'))} className="text-white/40 text-sm hover:text-white mb-1">← Kembali</button>
            ...
            <div className="flex gap-2 mt-1">
              ...
              <span className="text-white/40 text-sm self-center font-mono">{identity.account_handle}</span>
            </div>
          </div>
          ...
          <GlassCard title="Tambah Data Identitas">
            <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-3">
                <label className="block text-sm text-white/60 mb-1">Catatan</label>
                <textarea value={data.notes} onChange={(e) => setData('notes', e.target.value)}
                  className="w-full bg-white/5 border border-white/15 rounded-lg px-3 py-2 text-white placeholder-white/30 h-16 resize-none outline-none"
              ...
              <tbody>
                {records.data.map((r) => (
                  <tr key={r.id} className="border-b border-white/5 hover:bg-white/5">
                    <td className="py-3 px-3 text-white">{r.record_date}</td>
                    <td className="py-3 px-3 text-right text-white">{(r.impressions || 0).toLocaleString()}</td>
                    <td className="py-3 px-3 text-right text-white">{(r.engagements || 0).toLocaleString()}</td>
                    <td className="py-3 px-3 text-white/50 text-xs max-w-xs truncate">{r.notes || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {records.data.length === 0 && (
              <div className="text-center py-8 text-white/30">Belum ada data riwayat</div>
            )}
          </div>
        </GlassCard>
