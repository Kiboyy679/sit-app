    ...
    <AppLayout>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-on-surface">Verifikasi Arsip</h1>
            <p className="text-on-surface-variant text-sm">Periode: {period}</p>
          </div>
              ...
              <tbody>
                {results.map((r, i) => (
                  <tr key={i} className="border-b border-outline-variant/20 hover:bg-surface-container-low/50">
                    <td className="py-3 px-3 text-on-surface">{r.file}</td>
                    <td className="py-3 px-3">
                      <Badge variant={r.valid ? 'success' : 'danger'}>
                        {r.valid ? 'Valid' : 'Invalid'}
                      </Badge>
                    </td>
                    <td className="py-3 px-3 text-on-surface-variant/70 text-xs font-mono break-all">{r.expected || '-'}</td>
                    <td className="py-3 px-3 text-on-surface-variant/70 text-xs font-mono break-all">{r.actual || r.error || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
